...
if [[ -z $(conda env list | grep rnapenr) ]]; then
...
bg() {
    ...
    # DATABASE=$HOME/rnapenr/blastdb/*.fasta
    ...
    for i in $(cat "$OUTPUT_DIR"/sample_sheet.csv); do
        ...
        echo "ref_seq#target#segment#sample_id#num_total_reads#num_mapp_reads#avg_depth#depth_10x#depth_100x#depth_1000x#ref_cov#ncount#ncount_perc" | tr '#' '\t' > "$OUTPUT_DIR"/"$SAMPLE_ID".summary.tsv
    done
